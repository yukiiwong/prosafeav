...
import car_dreamer
from planet_agent import PlaNetAgent
...
def main(argv=None):
    ...
    config_flat = config_flat.update(planet_params)
    ...
    print(f"[Train PlaNet] Config saved to {config_file}")
    print("OBS SPACE:", env.obs_space)

    # === Initialize Agent and Training ===
    agent = PlaNetAgent(env.obs_space, env.act_space, dreamerv3_config)
