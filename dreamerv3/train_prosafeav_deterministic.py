...
def main(argv=None):
    ...
    # Add ProSafeAV-Deterministic parameters
    det_params = {
        "embed_dim": 64,
        "hidden_dim": 128,
        "model_lr": 1e-3,
    ...
    config_flat = config_flat.update(det_params)
    ...
    print(f"[Train ProSafeAV-Deterministic] Config saved to {config_file}")
    print("OBS SPACE:", env.obs_space)
    print("=" * 60)
    print("ABLATION STUDY: ProSafeAV-Deterministic")
    print("- NO stochastic latent variables (fully deterministic)")
    print("- NO sampling, NO KL divergence")
    print("- Testing if stochasticity is necessary")
    print("- EVT module handles uncertainty externally")
    print("=" * 60)

    # === Initialize Agent and Training ===
    agent = ProSafeAVDeterministicAgent(env.obs_space, env.act_space, dreamerv3_config)
