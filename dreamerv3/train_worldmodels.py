...
import car_dreamer
from world_models_agent import WorldModelsAgent
...
def main(argv=None):
    ...
    config_flat = config_flat.update(wm_params)
    ...
    print(f"[Train World Models] Config saved to {config_file}")
    print("OBS SPACE:", env.obs_space)

    # === Initialize Agent and Training ===
    agent = WorldModelsAgent(env.obs_space, env.act_space, dreamerv3_config)
